...
public sealed record GetAuditLogsQuery(int Page = 1, int PageSize = 20) : IRequest<PagedAuditLogsResponse>;
...
public sealed class GetAuditLogsQueryHandler(IAuditLogRepository repo) : IRequestHandler<GetAuditLogsQuery, PagedAuditLogsResponse>
{
    ...
    {
        var (items, totalCount) = await repo.GetPagedAsync(request.Page, request.PageSize, ct);
